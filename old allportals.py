    ...
    def next_sh(self):
        ...
        if self.strongholds.completed_8th_ring == 9 and not self.strongholds.empty_index:
            ...
            for i in range(len(self.strongholds.estimations)-self.strongholds.get_completed_count()+1): # check strongholds left in estimations for empty sector, elem contains sh object
                elem = self.strongholds.estimations[i+self.strongholds.get_completed_count()] #this had a +1 at the end before and gave me an error so i just got rid of it? probably only matters if the empty sector is your last sh
                if elem.get_ring()==8:
                    elem.set_empty(True)
