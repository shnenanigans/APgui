...
class FindPortals:
    ...
    #def __init__(self):
        ...
        self.topmost_toggle = tk.Checkbutton(self.toggle_frame, bg="#fee5b5", text="Always On Top", variable=top_value, onvalue=True, offvalue=False, command=lambda: [self.check_top(top_value.get())])
        ...
        self.next_button = tk.Button(self.window3, text="next", command=self.check_next, bg="#fecca8")
    ...
    def lock_entry(self, entry, lock, i):
        ...
        try:
            ...
            print(e)
            messagebox.showerror(message="Integer(s) only input. Seperate multiple numbers with spaces")
            return
    ...
    def check_next(self):
        if len(self.first_strongholds) == 8:
            ...
            #get rid of everything in the first-strongholds part
            ...
            self.topmost_toggle.config(bg="#cdeaf7")
            ...
            self.newnext_button = tk.Button(self.bt_frame, text="next", command=self.next_sh, height=1, width=5, borderwidth=3, bg="#99d4e9")
            ...
            self.manual_count = tk.Button(self.new_buttons_frame, text="Change stronghold count", command=self.override_count, borderwidth=3, bg="#99d4e9")
            self.forgot_spawn = tk.Button(self.new_buttons_frame, text="Forgot to set spawn", command=self.find_from_spawn, borderwidth=3, bg="#99d4e9")
            self.got_lost = tk.Button(self.new_buttons_frame, text="Pathfind from coords", command=self.find_from_coords, borderwidth=3, bg="#99d4e9")
    ...
    def next_sh(self):
        ...
        if self.count <= 128:
            ...
            self.sh_label.config(text=f"Stronghold {str(self.count)}:\n{str(self.sh_n)} at angle {str(self.ang)}", font=('Arial', 14))
            if self.sh in self.eighth_ring:
                ...
                self.empty_button = tk.Button(self.bt_frame, text="empty", command=self.empty, height=1, width=5, borderwidth=3, bg="#99d4e9")
...
#fix 8th ring stuff if you hit empty sector last
#make it faster surelyyyyy